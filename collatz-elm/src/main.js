...
var $author$project$Main$dictInsert = F2(
	function (d, lines) {
		dictInsert:
		while (true) {
			if (!lines.b) {
				...
				var h = lines.a;
				var t = lines.b;
				var _v1 = A2($elm$core$Dict$get, h.w, d);
				if (_v1.$ === 'Nothing') {
					var $temp$d = A3(
...
var $author$project$Main$odd = function (n) {
	return A2($elm$core$Basics$modBy, 2, n) === 1;
};
var $author$project$Main$collatz = function (n) {
	return (n === 1) ? _List_fromArray(
		[1]) : ($author$project$Main$odd(n) ? A2(
		$elm$core$List$cons,
		n,
		$author$project$Main$collatz((((n * 3) + 1) / 2) | 0)) : A2(
		$elm$core$List$cons,
		n,
	...
	if (czNode.$ === 'None') {
		...
		return _Utils_ap(
			...
			_Utils_ap(
				rb,
				_Utils_ap(
					$author$project$Main$collectLines(left),
					$author$project$Main$collectLines(right))));
	}
};
...
var $author$project$Main$toAlpha = F2(
	function (w, m) {
		return A2($elm$core$Basics$logBase, $elm$core$Basics$e, w) / A2($elm$core$Basics$logBase, $elm$core$Basics$e, m);
	});
...
var $author$project$Main$weightedLinesToPathSeg = function (wLines) {
	return $elm$core$List$concat(
		A2(
			...
			function (l) {
				return _List_fromArray(
					[
						$joakin$elm_canvas$Canvas$moveTo(l.from),
						$joakin$elm_canvas$Canvas$lineTo(l.to)
					]);
			},
			wLines));
};
var $author$project$Main$toLineShape = F3(
	function (w, max, l) {
		return A2(
			...
			_List_fromArray(
				[
					...
					_Utils_Tuple2(0, 0),
					$author$project$Main$weightedLinesToPathSeg(l))
				]));
	});
...
var $author$project$Main$toLineShapes = F2(
	function (d, max) {
		return A2(
			$elm$core$List$map,
			function (k) {
				return A3(
					$author$project$Main$toLineShape,
					k,
					max,
					A2(
